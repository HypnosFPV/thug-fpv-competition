...
export default async function MyEntriesPage() {
  ...
  const { data, error } = await supabase
    .from('entries')
    .select('id, title, moderation_status, moderation_notes, running_order, youtube_url, created_at, status_token, competitions(name)')
    .eq('user_id', user.id)
  ...
  return (
    <main className="page-shell page-stack">
      ...
      <section className="panel">
        ...
        {error && <div className="card error-card"><strong>Could not load entries</strong><p className="muted">{error.message}</p></div>}
        ...
        {data && data.length > 0 && (
          <div className="list">
            {data.map((entry) => {
              ...
              const comp = competitionName(entry);
              return (
                <div key={entry.id} className={`card ${entry.moderation_status === 'Approved' ? 'queue-now-card' : entry.moderation_status === 'Rejected' ? 'error-card' : 'notice-card'}`}>
                  ...
                  {comp && <p className="muted">Competition: {comp}</p>}
